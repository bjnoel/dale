...
//
// width/height are ATTRIBUTES, not just a CSS rule. This file is a static
// asset that deploy.sh copies straight across, while the stylesheet that sizes
// it is generated by build-dashboard.py into index.html. Deploy one without
// rebuilding the other and an SVG carrying only a viewBox falls back to its
// default 300x150, which is exactly what shipped on 2026-08-16: a pill the
// size of a dinner plate on every row. The CSS rule still wins where it loads
// (it sizes in em, so the glyph tracks the pill's text), but the attributes
// mean the worst case is a slightly-off icon rather than a broken page.
const BELL_SVG =
  '<svg class="watch-icon" width="16" height="16" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">' +
...
const CHART_DOWN_SVG =
  '<svg class="watch-icon" width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" ' +
