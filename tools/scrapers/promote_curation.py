...
def load_overrides(path: Path) -> dict:
    ...
    return {"deny": list(raw.get("deny") or []),
...
def merge(overrides: dict, pending: list) -> tuple[dict, list, list]:
    ...
    """
    new = {"deny": sorted(set(overrides["deny"])), "alias": dict(overrides["alias"])}
    applied, skipped = [], []
    for row in pending:
        ...
        if kind == ad.ALIAS:
            ...
            if new["alias"].get(slug) == target:
                skipped.append((row, "already aliased there"))
                continue
            ...
            if slug in new["deny"]:
                skipped.append((row, "already denied"))
                continue
...
def main() -> int:
    ...
    # Only now is the queue safe to clear: the decision is in git. Re-read first
    # so anything queued while the suite was running survives.
    fresh = ad.load_decisions(path)
    done = {r["from"] for r in applied}
    fresh["curation_pending"] = [r for r in (fresh.get("curation_pending") or [])
                                 if r.get("from") not in done]
    ad.save_decisions(path, fresh)
    print(f"Cleared {len(done)} promoted row(s) from the queue.")
