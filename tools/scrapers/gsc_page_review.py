...
def build_review_card(pages_data, report_date):
    """The <=100 word decision card that goes in the description.

    The full brief runs to ~800 words and used to be pasted straight into the
    description, which is most of why the backlog became unreadable. Benedict
    needs one line to decide whether this fortnight's review is worth a slot;
    the tables belong in the research comment.
    """
    pages = len(pages_data)
    opportunities = sum(len(p.get("opportunity_queries", [])) for p in pages_data)

    # Lead with the single best opportunity: highest impressions among pages
    # sitting just off page one, which is where a rank nudge pays most.
    best = None
    for p in pages_data:
        for q in p.get("opportunity_queries", []):
            if best is None or q["impressions"] > best[1]["impressions"]:
                best = (p, q)

    lines = [
        f"Fortnightly GSC review: {pages} pages, {opportunities} queries stuck at "
        f"position 11-30 where a small rank gain pays.",
        "",
    ]
    if best:
        page, q = best
        path = page["url"].replace(SITE_BASE, "") or "/"
        lines += [
            f"**Why now:** `{path}` ranks {q['position']:.0f} for "
            f"\"{q['keys'][0]}\" on {q['impressions']} impressions and "
            f"{q['clicks']} clicks.",
            "",
        ]
    lines += [
        "**Cost:** $0 · Dale autonomous.",
        "",
        "`L0 · treestock_organic_visitors`",
    ]
    return "\n".join(lines)


def post_to_linear(report_md, report_date, pages_data=None, dry_run=False):
    """Create a Linear ticket: decision card in the description, brief in a comment."""
    if dry_run:
        print(f"[DRY RUN] Would create Linear ticket: GSC page review - {report_date}")
        return None

    title = f"treestock: GSC page review - {report_date}"
    description = build_review_card(pages_data or [], report_date)
...
def run_review(pages=5, days=28, dry_run=False):
    ...
    # Post to Linear
    ticket_id = post_to_linear(report_md, review_date, pages_data=pages_data, dry_run=dry_run)
